...
- Recommended default: B. The dev's stated core tenant is unambiguous, and the test-leak failure mode is real (already happened on the rejection-flash-spec slug). Open-edit also matches the loop the GDD names ("Build it. Drive it. Build more.") where the implied subject is "anyone, including someone different from the original builder".
- Status: resolved
