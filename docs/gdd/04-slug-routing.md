...
- 2026-05-03: per-route metadata landed for the drive and edit pages (REQ-006, REQ-007, REQ-053). New pure helper module `src/app/[slug]/slugMetadata.ts` exports `APP_NAME = 'VibeCity'`, `TITLE_SEPARATOR = ' | '`, `driveTitle(slug)` ("Drive <slug> | VibeCity"), `editTitle(slug)` ("Edit <slug> | VibeCity"), `driveDescription(slug)` ("Drive around the <slug> city on VibeCity."), and `editDescription(slug)` ("Build the <slug> city on VibeCity."). `src/app/[slug]/page.tsx` and `src/app/[slug]/edit/page.tsx` now export `generateMetadata` that validates the slug via `parseSlugParam`, returns `{}` on rejection so the layout default still applies on a 404 render, and otherwise composes the per-route title / description plus matching `openGraph` and `twitter` cards so a shared link in iMessage, Slack, or a browser-tab list reads with the slug context instead of the bare site name. Files: `src/app/[slug]/slugMetadata.ts`, `src/app/[slug]/page.tsx`, `src/app/[slug]/edit/page.tsx`, `tests/app/slugMetadata.test.ts` (31 cases covering the title / description shape, role verbs, slug interpolation, and drive / edit divergence), `tests/app/driveRoute.test.ts` (6 new generateMetadata cases), `tests/app/editRoute.test.ts` (5 new generateMetadata cases), `e2e/drive.spec.ts` (1 new spec asserting the `<title>` reads "Drive title-spec-city | VibeCity"), `e2e/editor.spec.ts` (1 new spec asserting the `<title>` reads "Edit title-spec-city | VibeCity"). PR #N.
