...
2. Validate the builder id cookie (`builderId`, REQ-009) is present and well-formed. The cookie is identity-only; VibeCity is open-edit, so the route does not gate writes per cookie. Any visitor with a valid cookie can write to any slug.
...
- 2026-05-05: Open-edit pivot landed (REQ-014). Removed the per-slug owner gate from the PUT route so any visitor can edit any slug, matching the dev's stated core tenant ("anybody should be able to edit any city"). Files: `src/app/api/city/[slug]/route.ts` (dropped the `kv.get(cityOwner)` read, the `403 not owner` branch, and the first-claim `kv.set(cityOwner, builderId)` write; the route now validates the cookie for identity only and writes the version / latest / versions / index keys), `src/lib/kv.ts` (removed the `cityOwner` key factory entry; the literal `city:${slug}:owner` namespace is no longer read or written by VibeCity, orphan keys from prior writes stay in KV but are ignored), `tests/app/cityRoute.test.ts` (renamed "saves a valid city, claims ownership..." to "saves a valid city and returns the canonical hash" with the owner assertion dropped; "rejects a write from a different builder once a slug is owned" flipped to "lets any builder overwrite an existing city (open-edit)" asserting builder B's PUT returns 200 and `city:${slug}:latest` advances to B's hash; renamed "lets the owner overwrite their own city" to "lets the same builder overwrite their own city" with the owner assertion dropped; `clearSlug` and the per-test `fake.del` calls no longer touch the removed key), `tests/lib/kv.test.ts` (removed the `cityOwner is per-slug` assertion and the `cityOwner` entry from the `every key starts with city:` invariant), `tests/app/middleware.test.ts` (doc comment updated to reflect that the cookie is identity-only, not a write gate). Also updated `docs/gdd/05-identity.md` (REQ-009 status text now describes the cookie as identity-only) and `docs/gdd/99-out-of-scope.md` (the "slug ownership transfer" line is replaced by an "open-edit means there is no owner to transfer" line). Q-008 records the pivot from owner-gated to open-edit. PR #N.
