...
- Resolved: PR [#214](https://github.com/Randroids-Dojo/VibeCity/pull/214). 2026-05-10. Ported VibeRacer's `sampleArc45Local` and `sampleDiagonalLocal` recipes into `src/lib/trackPath.ts`. arc45 ships as a 25-sample cubic bezier from the south edge midpoint to the NE corner; diagonal ships as a 17-sample straight 45deg line from the SW corner to the NE corner with constant heading PI/4. `baseEntryDirAfterRotation` now takes a piece type so diagonal's SW base entry (`DIR_SW = 5`) participates in the reversal-on-opposite-end logic. The drive-scene procedural ribbon (PR #212) now extends through arc45 and diagonal pieces. Connector validation already recognized arc45's cardinal / corner pair via REQ-063 (`connectors.ts` `BASE_CONNECTOR_DIRS` entry).
...
- Resolved: PR [#214](https://github.com/Randroids-Dojo/VibeCity/pull/214). Implicitly resolved by the F-003 slice. `src/lib/wheelContact.ts` is fully agnostic to piece type and reads `ordered.samples` through the injected `distanceFn`, so once arc45 and diagonal populate `OrderedPiece.samples` (the F-003 slice), their wheel-contact path is wired without a per-type table to update. REQ-032 was already done (PR #210); this followup just needed the geometry layer to land.
...
- Status: slice 1 landed 2026-05-09 (`src/lib/editor/history.ts` re-exports from `@randroids-dojo/vibekit`) via PR https://github.com/Randroids-Dojo/VibeCity/pull/203.
- Status: slice 2 landed 2026-05-11 (`src/lib/controlsPersistence.ts` now calls the kit's `readStorage` / `writeStorage` / `removeStorage` directly; `src/lib/storage/localStorage.ts` + its test file removed). One behavioral delta on `clearControls`: the "returns false when localStorage throws on remove" path is no longer reachable because the kit swallows the throw; client branch returns true, SSR branch still returns false. See PR [#221](https://github.com/Randroids-Dojo/VibeCity/pull/221).
- Status: slice 3 landed 2026-05-12 (`src/lib/cityKv.ts` now re-exports `getKv` from `@randroids-dojo/vibekit/server` with a local `hasKvConfigured = () => getKv() !== null` synonym for back-compat; `src/lib/storage/kv.ts` removed; all 4 lib + route callers switched to the `const kv = getKv(); if (!kv) return ...` pattern). Behavioral delta: `getKv()` now returns `null` on missing env instead of throwing. The `cityKv` test surface gained `resetKvForTesting()` in beforeEach/afterEach so kit caching does not bleed across tests, and the "throws when env is unset" case flipped to "returns null when env is unset". See PR [#222](https://github.com/Randroids-Dojo/VibeCity/pull/222).
...
- Resolved: PR #249. 2026-05-18. Slice 1 (PR #247) shipped the substrate (`cellHappiness` pure resolver). Slice 2 (this PR) wires resident decline. New `applyHappinessDecline(population, zones, water, power, services, taxRates, disasters)` in `src/lib/sim/events.ts` walks every populated residential cell, computes per-cell happiness, and advances a per-cell `unhappyTicks` counter on `PopulationCell` (optional schema field, defaults 0). When the counter reaches `CELL_DECLINE_TICKS_TO_LOSE_RESIDENT = 5` AND density > 0, the zone density drops by 1 and the counter resets. The next `syncPopulationToZones` reduces `residents` to the lower capacity automatically. Constants: `CELL_DECLINE_HAPPINESS_THRESHOLD = 40` (well below the city growth-stall threshold of 50 so cells get a stalled-but-stable band before decline kicks in). Identity-on-no-change. Called from `applyTick` on growth ticks only, before the existing `syncPopulationToZones`, so the new (lower) density is what gets synced. The abandoned-cell visual stroke + city-wide happiness penalty already in place light up automatically when density hits 0.
...
- Resolved: PR #225. Picked option (d), unscoped at the time this followup was written: the prebuilt demo city at `/demo` bypasses KV in `loadCity.ts` (returns `DEMO_CITY` directly when no save exists) and gives the playwright webServer a populated 24-piece city without any fixture or seed handler. New `F-008` spec in `e2e/drive.spec.ts` navigates to `/demo/drive`, asserts the populated branch (`data-vehicle="true"`, `data-controls-active="true"`, `data-camera-mode="chase"`), presses Escape to unpause the press-to-start menu, then holds `KeyW` for 1s and polls `data-car-x` / `data-car-z` / `data-car-speed` to assert the integrator actually advanced the car.
...
- Resolved: PR #156. New `src/lib/cityThumbnail.ts` exports `cityThumbnailDots(city)` that projects pieces and buildings into a normalized `[0, 1] x [0, 1]` dot list. `src/app/page.tsx` fans out one `loadCity()` per recent slug (bounded at the 12 default) and renders a fixed-size `home-recent-thumbnail` 60x60 SVG inside each recent-card alongside the slug + relative-time labels. Pieces render as dark brown dots, buildings as warm tan dots; an empty city renders the background with no dots. Per-zone-kind color, multi-cell footprint walking, and per-version-hash render caching stay deferred under F-NEW.
