...
- Resolved: PR #136. Switched both `computeCityHappiness` and `applyHappinessTick` to call the per-key `cellCoverage(row, col, services)` helper. Public signatures dropped the `zones` parameter; per-tick cost is now `O(R * |services|)`. Numerical equivalence proved by the existing 2144/2144 unit suite passing unchanged.
