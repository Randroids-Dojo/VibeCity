...
- Status: slice 2 landed 2026-05-11 (`src/lib/controlsPersistence.ts` now calls the kit's `readStorage` / `writeStorage` / `removeStorage` directly; `src/lib/storage/localStorage.ts` + its test file removed). One behavioral delta on `clearControls`: the "returns false when localStorage throws on remove" path is no longer reachable because the kit swallows the throw; client branch returns true, SSR branch still returns false. See PR TBD.
